...
class GenerativeQAModule(BaseTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        # #if you want to freeze question encoder
    ...
    def training_step(self, batch, batch_idx) -> Dict:
        source_ids, source_mask, target_ids = batch["input_ids"], batch["attention_mask"], batch["decoder_input_ids"]
#         print("source_ids type:", type(source_ids))
#         print("source_ids value:", source_ids)

#         print("source_mask type:", type(source_mask))
#         print("source_mask value:", source_mask)
        
#         ########add###############################################
        
#         # 批量解码原文本
#         source_text =self.tokenizer.question_encoder.batch_decode(source_ids, skip_special_tokens=True)
#         target_text = self.tokenizer.generator.batch_decode(target_ids, skip_special_tokens=True)
...
#         question_hidden_states = self.model.question_encoder(source_ids,source_mask)[0]
#         question_hidden_states_np = question_hidden_states.detach().cpu().numpy()

#         print(question_hidden_states_np)

#         # 2. Retrieve documents
#         docs = self.model.retriever(
#         question_hidden_states_np, 5)[2]
#         ##########################################################
...
#         # Combine and average training metrics
#         avg_train_metrics = {name: torch.stack([x["log"][name] for x in outputs]).mean() for name in self.loss_names}

#         # Convert tensor values to Python native types
#         avg_train_metrics = {k: v.item() if isinstance(v, torch.Tensor) else v for k, v in avg_train_metrics.items()}

#         # Save training metrics
#         self.save_metrics(avg_train_metrics, "train")
    ...
    def training_epoch_end(self, outputs) -> None:
        ...
        # Distributed training: reduce metrics across all processes
        if dist.is_initialized():
            for k in avg_train_metrics:
                dist.all_reduce(avg_train_metrics[k], op=dist.ReduceOp.SUM)
                avg_train_metrics[k] /= dist.get_world_size()
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        ...
        self.log('val_em', metrics_tensor.item())     #add
...
def main(args=None, model=None) -> GenerativeQAModule:
    ...
    trainer: pl.Trainer = generic_train(
        ...
        checkpoint_callback=get_checkpoint_callback(args.output_dir, model.val_metric_loss),
        # checkpoint_callback=get_checkpoint_callback(args.output_dir, model.val_metric),     #change
